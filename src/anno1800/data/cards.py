...
def create_population_deck() -> PopulationCardDeck:
    ...
    for template in POPULATION_CARDS:
        card = deepcopy(template)

        if card.population_type in (PopulationType.FARMER, PopulationType.WORKER):
            farmer_worker_cards.append(card)
        else:
            advanced_cards.append(card)

    return PopulationCardDeck(farmer_worker_cards=farmer_worker_cards, advanced_cards=advanced_cards)
